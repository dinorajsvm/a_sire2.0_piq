...
import { ApiMethod, colorCodes, ErrorCodes, ErrorMessage } from '../../constants';
...
export class HttpService {
  ...
  constructor(
    ...
    private _snackBar:SnackbarService, private _storage:StorageService) { }
  ...
  requestCall(api: string, method: ApiMethod, data?: any, definedApiUrl?:string) { 
    ...
    switch (method) { 
      ...
      case ApiMethod.POST:
        response = this.http.post(`${apiUrl}${api}`, data)
        .pipe(catchError((err) => this.handleError(err, this)));
        break;
      ...
      case ApiMethod.POST_WITH_HEADER:
        ...
        case ApiMethod.GET_WITH_HEADER:
          response = this.http.get(`${apiUrl}${api}`, { 
            headers: new HttpHeaders({'authorization': this._storage.getAccessToken()})
          }).pipe(catchError((err) => this.handleError(err, this)));
          break;

      case ApiMethod.PUT:
        response = this.http.put(`${apiUrl}${api}`, data)
          .pipe(catchError((err) => this.handleError(err, this)));
        break;
  ...
  whichError(errorCode: number, errorMessage: string="") {
    switch (errorCode) {
      ...
      case ErrorCodes.HTTP_CONN_REFUSED:
        this._snackBar.loadSnackBar(ErrorMessage.NETWORK_ERROR, colorCodes.ERROR);
        break;
